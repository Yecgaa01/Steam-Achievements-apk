...
class _GameDetailScreenState extends State<GameDetailScreen> {
  ...
  AppText get t => AppText(widget.config.languageCode == 'en' ? AppLanguage.english : AppLanguage.portuguese);
  ...
  Future<List<SteamAchievement>> _loadAchievements() async {
    final cached = await _cache.loadCachedAchievements(widget.config.normalizedSteamId64, widget.game.appId);
    if (cached.isNotEmpty) {
      if (mounted) setState(() => _showingOfflineCache = true);
      _refreshAchievementsInBackground();
      return cached;
    }
    final achievements = await _api.getAchievements(widget.config, widget.game.appId);
    if (achievements.isNotEmpty) {
      await _cache.saveCachedAchievements(widget.config.normalizedSteamId64, widget.game.appId, achievements);
    }
  ...
  List<SteamAchievement> _applyFilter(List<SteamAchievement> achievements) {
    ...
    return achievements.where((achievement) {
      ...
      return achievement.name.toLowerCase().contains(query) || achievement.description.toLowerCase().contains(query) || achievement.apiName.toLowerCase().contains(query);
    }).toList();
  }

  int _firstAchievementUnlockTime(List<SteamAchievement> achievements) {
    final unlockTimes = achievements.where((achievement) => achievement.achieved && achievement.unlockTime > 0).map((achievement) => achievement.unlockTime).toList();
  ...
  String _formatUnixDate(int unixSeconds) {
    ...
    final date = DateTime.fromMillisecondsSinceEpoch(unixSeconds * 1000, isUtc: true).toLocal();
  ...
  String? _progressTierLabel() {
    if (!widget.config.showProgressTiers || !widget.game.progressLoaded || !widget.game.hasAchievements) return null;
  ...
  Color _progressTierColor(String label) {
    ...
    if (label == t.tierSilver) return dark ? const Color(0xFFC0C0C0) : const Color(0xFF8A8F98);
    if (label == t.tierGold) return dark ? const Color(0xFFFFD700) : const Color(0xFFB8860B);
    if (label.contains(t.tierPerfect)) return dark ? const Color(0xFFB9F2FF) : const Color(0xFF64748B);
  ...
  Widget _dateLine(String label, int unixSeconds) {
    return Text(
      ...
      style: const TextStyle(color: Colors.white70, fontSize: 12.5, fontWeight: FontWeight.w600, shadows: [Shadow(color: Colors.black87, blurRadius: 8)]),
    );
  }
    ...
    return Scaffold(
      body: FutureBuilder<List<SteamAchievement>>(
        ...
        builder: (context, snapshot) {
          ...
          final unlocked = achievements.where((achievement) => achievement.achieved).length;
          final total = achievements.length;
          final progress = total == 0 ? 0.0 : unlocked / total;
          final firstAchievementUnlockTime = _firstAchievementUnlockTime(achievements);
          ...
          return CustomScrollView(
            slivers: [
              SliverAppBar(
                expandedHeight: 225,
                pinned: true,
                title: Text(widget.game.name, maxLines: 1, overflow: TextOverflow.ellipsis),
                flexibleSpace: FlexibleSpaceBar(
                  background: Stack(
                    fit: StackFit.expand,
                    children: [
                      CachedNetworkImage(imageUrl: widget.game.headerUrl, fit: BoxFit.cover),
                      ...
                      Positioned(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(widget.game.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900)),
                            const SizedBox(height: 8),
                            TrophyProgressBar(value: progress, height: 10, valueColor: widget.config.goldPerfectGames && progress >= 1 ? const Color(0xFFFACC15) : null),
                            const SizedBox(height: 8),
                            Text('${t.achievementsProgress(unlocked, total)} • ${(progress * 100).round()}%', style: const TextStyle(color: Colors.white70)),
                            if (_progressTierLabel() != null) ...[
                              const SizedBox(height: 3),
                              Builder(builder: (context) {
                                final tier = _progressTierLabel()!;
                                return Text(tier, style: TextStyle(color: _progressTierColor(tier), fontSize: 12.5, fontWeight: FontWeight.w900, shadows: const [Shadow(color: Colors.black87, blurRadius: 8)]));
                              }),
                            ],
                            const SizedBox(height: 6),
                            _dateLine(t.firstAchievement, firstAchievementUnlockTime),
                            const SizedBox(height: 3),
                            _dateLine(t.lastPlayed, widget.game.lastPlayedUnix),
                          ],
                        ),
                      ),
                    ],
              ...
              if (_showingOfflineCache)
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: Text(t.offlineAchievementsCache, style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
                  ),
                ),
              SliverToBoxAdapter(
                child: Padding(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      FilterChip(
                        ...
                        onSelected: (value) => setState(() => _showHiddenLocal = value),
                      ),
                    ],
                  ),
                ),
              ...
              if (snapshot.connectionState != ConnectionState.done)
                ...
                SliverFillRemaining(child: Center(child: Text(t.noAchievementForFilter)))
              else
                SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      return AchievementTile(achievement: item as SteamAchievement, text: t, revealHiddenByDefault: _showHiddenLocal, showRarityTier: widget.config.showRarityTiers);
                    },
                    childCount: _buildAchievementRows(visibleAchievements).length,
                  ),
                ),
            ],
          );
  ...
  List<Object> _buildAchievementRows(List<SteamAchievement> achievements) {
    final hasGroups = widget.config.separateDlcAchievements && achievements.any((achievement) => achievement.groupName.isNotEmpty);
    if (!hasGroups) return achievements;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(t.loadAchievementsFailed, style: const TextStyle(fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
